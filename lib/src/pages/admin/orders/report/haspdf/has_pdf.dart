...
import 'has_pdf_controller.dart';

class hasPDF extends StatefulWidget {
  const hasPDF({super.key});

  @override
  State<hasPDF> createState() => _hasPDFState();
}

class _hasPDFState extends State<hasPDF> {
  HasPDFController _con = new HasPDFController();
  Uint8List? _pdfBytes;
  @override
  void initState() {
    ...
    return Container(
      ...
      child: ElevatedButton(
        onPressed: () async {
          ...
          List<UserHasRoles> roleshas = await _con.getAlluser_has_roles();

          // Generar el PDF
          Uint8List pdfBytes = await _con.generatePdfBytes(roleshas);

          // Guardar el PDF en el directorio temporal del dispositivo
          String fileName = "roleshas_report.pdf";
